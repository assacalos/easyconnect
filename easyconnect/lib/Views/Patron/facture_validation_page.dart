...
class _FactureValidationPageState extends State<FactureValidationPage>
          ...
          Padding(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Rechercher par numéro de facture...',
  ...
  Widget _buildInvoiceList() {
    ...
    final filteredInvoices =
        _searchQuery.isEmpty
            ...
            : controller.invoices
                .where(
                  (facture) => facture.invoiceNumber.toLowerCase().contains(
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final facture = filteredInvoices[index];
        return _buildFactureCard(context, facture);
      },
    );
  }

  Widget _buildFactureCard(BuildContext context, InvoiceModel facture) {
      ...
      child: ExpansionTile(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            Text('Date: ${formatDate.format(facture.invoiceDate)}'),
            Text('Montant: ${formatCurrency.format(facture.totalAmount)}'),
            const SizedBox(height: 4),
            Container(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text('Client: ${facture.clientName}'),
                      Text('Email: ${facture.clientEmail}'),
                      Text('Adresse: ${facture.clientAddress}'),
                    ],
                  ),
                ),
                const SizedBox(height: 16),
                ...
                const SizedBox(height: 8),
                ...facture.items.map((item) => _buildItemDetails(item)),
                const SizedBox(height: 16),
                Container(
                  padding: const EdgeInsets.all(12),
                  decoration: BoxDecoration(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          Text(formatCurrency.format(facture.subtotal)),
                        ],
                      ),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          const Text('TVA:'),
                          Text(formatCurrency.format(facture.taxAmount)),
                        ],
                      ),
                      const Divider(),
                      Row(
                        ...
                        children: [
                          ...
                          Text(
                            formatCurrency.format(facture.totalAmount),
                            style: const TextStyle(fontWeight: FontWeight.bold),
                          ),
                        ],
                      ),
                ...
                const SizedBox(height: 16),
                _buildActionButtons(facture, statusColor),
              ],
            ),
          ),
        ],
  ...
  Widget _buildActionButtons(InvoiceModel facture, Color statusColor) {
      ...
      return Column(
        children: [
          Row(
            ...
            children: [
              ElevatedButton.icon(
                onPressed: () => _showApproveConfirmation(facture),
              ...
              ElevatedButton.icon(
                onPressed: () => _showRejectDialog(facture),
    ...
    } else if (facture.status == 'sent' || facture.status == 'paid') {
  ...
  void _showApproveConfirmation(InvoiceModel facture) {
    Get.defaultDialog(
      title: 'Confirmation',
      ...
      onConfirm: () {
        Get.back();
        controller.approveInvoice(facture.id);
        _loadInvoices();
      },
    );
  }

  void _showRejectDialog(InvoiceModel facture) {
    ...
    Get.defaultDialog(
      ...
      onConfirm: () {
        ...
        Get.back();
        controller.rejectInvoice(facture.id, commentController.text);
        _loadInvoices();
      },
    );
  }
