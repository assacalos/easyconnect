  ...
  Future<void> loadClients({
    ...
    if (page == 1) {
      ...
      if (cachedData.isNotEmpty) {
        ...
        // Ne pas return : on rafraîchit en arrière-plan
      } else {
        state = state.copyWith(isLoading: true);
        // Ne jamais vider state.clients : garder l'ancienne liste ou vide + skeleton
      }
    } else {
    ...
    try {
      AppLogger.debug(
        'loadClients: appel API status=$status page=$page',
        tag: 'CLIENT_NOTIFIER',
      );
      ...
      AppLogger.debug(
        'loadClients: OK ${response.data.length} client(s) page=$page',
        tag: 'CLIENT_NOTIFIER',
      );
      ...
      final is401 = errStr.contains('401') ||
          ...
          errStr.contains('non autorisé');
      AppLogger.error(
        'Erreur API Clients: $e (401/auth=$is401)',
        tag: 'CLIENT_NOTIFIER',
        error: e,
