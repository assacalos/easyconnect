...
class CommercialDashboardNotifier extends AsyncNotifier<CommercialDashboardState> {
  ...
  /// [silent] : si true, ne met pas isLoading à true (rafraîchissement en arrière-plan).
  ...
  Future<List<T>> _safeLoadList<T>(Future<List<T>> Function() fn, String label) async {
    try {
      ...
      AppLogger.warning(
        'Dashboard commercial: chargement $label échoué: $e',
        tag: 'COMMERCIAL_DASHBOARD_RIVERPOD',
      );
      return [];
    }
  ...
  Future<CommercialDashboardState> _loadAll(
    ...
    try {
      ...
      final results = await Future.wait([
        _safeLoadList(() => clientService.getClients(), 'clients'),
        _safeLoadList(() => devisService.getDevis(), 'devis'),
        _safeLoadList(() => bordereauService.getBordereaux(), 'bordereaux'),
        _safeLoadList(() => bonCommandeService.getBonCommandes(), 'bon_commandes'),
        _safeLoadList(() => bonCommandeFournisseurService.getBonDeCommandes(), 'bon_commandes_fournisseur'),
      ]);
      ...
      try {
        ...
        AppLogger.warning('Dashboard commercial: tâches échoué: $e', tag: 'COMMERCIAL_DASHBOARD_RIVERPOD');
      }

      final revenue = devis.fold<double>(0.0, (sum, d) => sum + (d.totalTTC as num).toDouble());
      final pendingDevisAmount = devis
          .where((d) => d.status == 1)
          .fold<double>(0.0, (sum, d) => sum + (d.totalTTC as num).toDouble());
      final paidBordereauxAmount = bordereaux
          .where((b) => b.status == 2)
          .fold<double>(0.0, (sum, b) => sum + (b.montantTTC as num).toDouble());
      s = s.copyWith(
