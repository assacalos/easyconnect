...
import 'package:easyconnect/Views/Client/client_request_ticket_page.dart';
import 'package:easyconnect/Views/Client/client_announcements_page.dart';
import 'package:easyconnect/Views/Client/client_catalog_page.dart';
import 'package:easyconnect/Views/Client/client_offers_page.dart';
import 'package:easyconnect/Views/Client/client_contact_page.dart';
import 'package:easyconnect/Views/Client/client_my_tickets_page.dart';
...
import 'package:easyconnect/Views/Comptable/inventory_list_page.dart';
  ...
  rootGoRouter = GoRouter(
    ...
    redirect: (BuildContext context, GoRouterState state) {
      ...
      if (user == null && !_isPublicRoute(loc)) {
        AppLogger.debug(
          'Redirect vers /splash: user=null, loc=$loc',
          tag: 'ROUTER',
        );
        return '/splash';
      }
    ...
    routes: [
      ...
      GoRoute(path: '/client', builder: (_, __) => const ClientDashboardPage()),
      GoRoute(path: '/client/request-ticket', builder: (_, __) => const ClientRequestTicketPage()),
      GoRoute(path: '/client/announcements', builder: (_, __) => const ClientAnnouncementsPage()),
      GoRoute(path: '/client/catalog', builder: (_, __) => const ClientCatalogPage()),
      GoRoute(path: '/client/offers', builder: (_, __) => const ClientOffersPage()),
      GoRoute(path: '/client/contact', builder: (_, __) => const ClientContactPage()),
      GoRoute(path: '/client/my-tickets', builder: (_, __) => const ClientMyTicketsPage()),
      ...
      GoRoute(path: '/comptable/inventaire', builder: (_, __) => const InventoryListPage()),
      GoRoute(
        path: '/comptable/inventaire/:id',
        builder: (_, s) {
          final id = int.tryParse(s.pathParameters['id'] ?? '0') ?? 0;
          final session = s.extra;
          return InventorySessionDetailPage(
            sessionId: id,
            session: session is InventorySession ? session : null,
          );
        },
      ),
