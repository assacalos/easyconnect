...
class NotificationService
{
    ...
    /**
     * Rappel personnel technicien : notification au technicien lui-même (J-1, J-2 ou J-3 avant la date limite).
     */
    public function notifyTechnicianPersonalReminder($reminder, int $daysLeft)
    {
        $reminder->loadMissing(['client', 'user']);
        $company = $reminder->client
            ? ($reminder->client->nom_entreprise ?? $reminder->client->nom ?? 'Client #' . $reminder->client_id)
            : ($reminder->company_name ?? 'Entreprise');
        $message = "Rappel : {$reminder->title} pour {$company} – date limite dans {$daysLeft} jour(s).";
        $this->createAndBroadcast(
            $reminder->user_id,
            'technician_reminder',
            'Rappel perso : ' . $reminder->title,
            $message,
            [
                'reminder_id' => $reminder->id,
                'entity_type' => 'technician_reminder',
                'entity_id' => (string) $reminder->id,
                'action_route' => '/mes-rappels',
            ],
            'normale'
        );
    }
