...
class JournalController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($request->filled('annee') && $request->filled('mois')) {
                ...
                $soldeInitial = JournalEntry::soldeAu($avant->toDateString());
            ...
            $lignes = $entries->map(function ($entry) use (&$solde) {
                ...
                $user = $entry->relationLoaded('user') && $entry->user
                    ? ['id' => $entry->user->id, 'nom' => $entry->user->nom, 'prenom' => $entry->user->prenom]
                    : null;
                return [
                    'id' => $entry->id,
    ...
    public function list(Request $request)
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'data' => JournalEntryResource::collection($paginated->items())->resolve(),
                'pagination' => [
    ...
    {
        try {
            $entry = JournalEntry::with('user')->find($id);
            if (!$entry) {
                return response()->json([
                    'success' => false,
